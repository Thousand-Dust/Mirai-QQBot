...
class QWeather(private val key: String) {
     ...
     * 获取状态码枚举
     */
    fun getStatusCode(code: String): QWeatherCode? {
        return QWeatherCode.values().find { it.code == code }
    }
    ...
    fun threeDayWeather(location: String): JSONObject? {
        ...
        params["key"] = key
        params["location"] = location
        val result = HttpUtils.get(HttpUrl.SevenDayWeather, params) ?: return null
        return JSONObject.parseObject(result.string())
    }
